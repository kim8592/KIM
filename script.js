...
const exportSMAS = async () => {
  const allTextareas = Array.from(document.querySelectorAll("textarea")).filter(area => {
    // Lọc 1: Ô phải đang hiển thị trên màn hình
    return area.offsetWidth > 0 && area.offsetHeight > 0;
  });

  const dataString = allTextareas
    .map(area => {
      const nhanXet = area.value.trim();
      
      // Nếu ô hoàn toàn trống, trả về null để lát nữa lọc bỏ
      if (!nhanXet) return null;

      let mucDat = "T"; 
      const row = area.closest('tr') || area.parentElement;
      
      // Tìm nút mức đạt đang được chọn
      const activeBtn = Array.from(row.querySelectorAll('button')).find(btn => 
        !btn.className.includes('text-slate-400') && btn.innerText.length <= 2
      );
      if (activeBtn) mucDat = activeBtn.innerText.trim();

      return `${mucDat}||${nhanXet}`;
    })
    // Lọc 2: Xóa bỏ những dòng bị null (dòng trống)
    .filter(line => line !== null) 
    .join('\n');

  try {
    await navigator.clipboard.writeText(dataString);
    alert("Bạn có đồng ý chuyển dữ liệu sang SMAS?");
  } catch (err) {
    alert("Lỗi copy!");
  }
};
...
comment = processComment(comment, level);
  ...
  {/* 1. NÚT CÀI ĐẶT: py-3 (cao hơn py-2.5 của các nút khác), px-2 (ngắn lại) */}
  <a
    href={`javascript:(async()=>{try{const t=await navigator.clipboard.readText();if(!t)return;const k=prompt('Nhap 1: Giua ki, 2: Cuoi ki','1');if(!k)return;const lines=t.trim().split('\\n');const rows=Array.from(document.querySelectorAll('tr')).filter(r=>r.querySelector('input.k-input'));let j=0;rows.forEach((row)=>{if(j>=lines.length)return;const mSuffix=(k==='1')?'-1':'-3';const cSuffix=(k==='1')?'_2':'_5';const inp=row.querySelector('input[id$=\"'+mSuffix+'\"]');const tx=row.querySelector('textarea[id$=\"'+cSuffix+'\"]');if(!inp||inp.disabled||inp.readOnly||(tx&&tx.disabled)){console.log('Bo qua HS chuyen truong');return}const p=lines[j].split('||');if(p.length<2){j++;return}if(inp){inp.value=p[0].toUpperCase();inp.dispatchEvent(new Event('input',{bubbles:true}))}if(tx){const set=Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype,'value').set;set.call(tx,p[1]);tx.dispatchEvent(new Event('input',{bubbles:true}));const sp=row.querySelector('[id$=\"'+cSuffix+'\"][id^=\"span_\"]');if(sp)sp.innerText=p[1]}j++});alert('Da xong!');}catch(e){alert('Loi: '+e)}})();`}
    onClick={(e) => e.preventDefault()}
    title="Nhấn giữ và kéo lên thanh dấu trang"
    className="relative flex items-center justify-center px-2 py-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-l-lg text-[10px] font-black uppercase transition-all cursor-move overflow-hidden border-r border-emerald-500/30 shadow-lg active:scale-95 z-10"
  >
  ...
  {/* 2. Nhóm chọn hệ thống (SMAS - VNEDU): Giữ nguyên py-2.5 */}
  {['smas', 'vnedu'].map((sys) => (
    <button 
      key={sys} 
      onClick={() => { setSystemMode(sys); setSelectedCriteriaId(''); setDraftData({}); }} 
      className={`px-6 py-2.5 text-[11px] font-black uppercase transition-all ${systemMode === sys ? 'bg-white text-indigo-900 shadow-lg' : 'text-indigo-300 hover:text-white'}`}
    >
      🌐 {sys}
    </button>
  ))}
  ...
  <div className="flex items-center">
    ...
    <button 
      ...
      className={`flex items-center gap-2 px-4 py-2.5 text-[11px] font-black uppercase transition-all shadow-md active:scale-95 ${draftCount > 0 ? 'bg-amber-500 text-white hover:bg-amber-600 animate-pulse' : 'bg-indigo-800 text-indigo-400 opacity-50 cursor-not-allowed'}`}
    >
    ...
    <button 
      ...
      className={`p-2.5 rounded-r-lg transition-all ${apiKey ? 'bg-emerald-600 text-white' : 'bg-red-500 text-white animate-bounce'} shadow-md border-l border-white/10`}
    >
      ⚙️
    </button>
  </div>
