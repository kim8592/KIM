...
function autoFixComment(level, comment) {
  ...
  // Phân chia T+C vs H
  const getTCLevelList = () => {
    if (viewMode === 'subject') return ['T', 'C'];
    else if (systemMode === 'vnedu' && viewMode !== 'subject') return ['T', 'C'];
    else return ['T', 'C'];
  };

  const hLevelList = ['H', 'Đ'];
  const tcLevels = getTCLevelList();

  const tcStudents = allTargets.filter(stu => {
    const d = studentData[stu.id] || {};
    const draft = draftData[stu.id] || {};
    if (viewMode === 'subject' || systemMode === 'vnedu') {
      const level = draft.level !== undefined ? draft.level : (d.level || "");
      return tcLevels.includes(level);
    } else {
      const list = viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES);
      return list.some(c => {
        const level = draft[`level_${c.id}`] !== undefined ? draft[`level_${c.id}`] : (d[`level_${c.id}`] || "");
  ...
  const hStudents = allTargets.filter(stu => {
    const d = studentData[stu.id] || {};
    const draft = draftData[stu.id] || {};
    if (viewMode === 'subject' || systemMode === 'vnedu') {
      const level = draft.level !== undefined ? draft.level : (d.level || "");
      return hLevelList.includes(level);
    } else {
      const list = viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES);
      return list.some(c => {
        const level = draft[`level_${c.id}`] !== undefined ? draft[`level_${c.id}`] : (d[`level_${c.id}`] || "");
  ...
  if (tcStudents.length === 0 && hStudents.length === 0) {
    showToast('Không có học sinh nào cần nhận xét', 'info', '⚠️', 4000);
    return;
  }
  ...
  if (tcStudents.length > 0) {
    showToast(`Đang tạo nhận xét T+C cho ${tcStudents.length} học sinh...`, 'info', '⏳', 2000);
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    const success = await generateCommentsForGroup(tcStudents, 'TC');
    if (!success) {
      setIsGenerating(false);
      return;
    }

    showToast(`Xong T+C! Đang nhận xét H (${hStudents.length} học sinh)...`, 'info', '⏳', 2000);
    await new Promise(resolve => setTimeout(resolve, 1500));
  }

  // ===== LƯỢT 2: NHẬN XÉT H =====
  if (hStudents.length > 0) {
    showToast(`Đang tạo nhận xét H cho ${hStudents.length} học sinh...`, 'info', '⏳', 2000);
    await new Promise(resolve => setTimeout(resolve, 1000));
    
    await generateCommentsForGroup(hStudents, 'H');
  }

  setIsGenerating(false);
  showToast(`Hoàn tất! TC: ${tcStudents.length}, H: ${hStudents.length}`, 'success', '✅', 3000);
};

const generateCommentsForGroup = async (groupStudents, groupType) => {
  try {
    // ===== TẠO DANH SÁCH SINH VIÊN (JSON) =====
    const studentList = groupStudents.map((stu) => {
      const d = studentData[stu.id] || {};
      const draft = draftData[stu.id] || {};
...
ĐỊNH DẠNG TRẢ VỀ (BẮT BUỘC - JSON):
{
  "comments": [
    {
      "studentId": "id_của_học_sinh",
      "studentName": "tên_học_sinh",
...
${JSON.stringify(studentList, null, 2)}`;
    ...
    let parsedData = null;
    try {
      const jsonMatch = responseText.match(/\{[\s\S]*"comments"[\s\S]*\}/);
      if (jsonMatch) {
      ...
      showToast('Cannot parse AI response', 'error', '❌', 4000);
      return false;
    }

    const comments = parsedData?.comments || [];
    if (!Array.isArray(comments) || comments.length === 0) {
      console.error('Invalid comments array:', parsedData);
      showToast('Invalid AI response format', 'error', '❌', 4000);
      return false;
    }
    ...
    comments.forEach(item => {
      if (item.studentId && item.comment) {
        ...
        const student = groupStudents.find(s => s.id === item.studentId);
        if (student) {
        ...
        const d = studentData[item.studentId] || {};
        const draft = draftData[item.studentId] || {};
        let level = "";
        ...
        if (comment.length > 10) {
          updates[item.studentId] = comment;
          successCount++;
        }
      }
    });
    ...
    if (successCount === 0) {
      console.error('Cannot extract comments:', parsedData);
      showToast('Cannot extract valid comments', 'error', '❌', 4000);
      return false;
    }
    ...
    console.log(`✅ Done group ${groupType}: ${successCount}/${groupStudents.length}`);
    return true;
                ...
                <div className="flex-1 w-full text-left">
                  ...
                  <div className="relative">
                    ...
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-xl pl-12 pr-4 py-4 text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                      placeholder="VD: hãy chọn 1 trong các nội dung sau: giải toán...tìm x...nhân...đặt tính...diện tích. khoảng 20 từ " 
                      value={aiPrompt} 
