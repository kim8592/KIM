...
const Toast = ({ message, type, icon }) => (
  <div className={`fixed top-4 right-4 sm:top-6 sm:right-6 z-[200] toast ${type === 'success' ? 'bg-emerald-50 border-emerald-300' : type === 'error' ? 'bg-red-50 border-red-300' : 'bg-blue-50 border-blue-300'} border-2 rounded-xl p-3 sm:p-4 shadow-2xl max-w-xs sm:max-w-sm`}>
    <div className="flex items-start gap-3">
      <span className="text-xl sm:text-2xl mt-0.5">{icon}</span>
      <div className="flex-1 min-w-0">
        <p className={`font-black text-xs sm:text-sm ${type === 'success' ? 'text-emerald-900' : type === 'error' ? 'text-red-900' : 'text-blue-900'} uppercase tracking-wide`}>
          {type === 'success' ? 'Thành công' : type === 'error' ? 'Lỗi' : 'Thông báo'}
        </p>
        <p className={`text-xs sm:text-sm mt-1 break-words ${type === 'success' ? 'text-emerald-700' : type === 'error' ? 'text-red-700' : 'text-blue-700'}`}>
          {message}
        </p>
      </div>
    </div>
...
const EditableCell = ({ value, onSave, className, isDraft, disabled }) => {
  ...
  return (
    <textarea
      ref={textareaRef}
      disabled={disabled}
      className={`${className} ${isDraft ? 'bg-yellow-50 shadow-inner' : ''} ${disabled ? 'opacity-30 cursor-not-allowed' : 'hover:bg-slate-50 focus:bg-white'} overflow-hidden whitespace-pre-wrap break-words min-h-[40px] p-2 rounded transition-colors w-full resize-none text-xs sm:text-sm`}
      rows={1}
      value={localValue}
...
const SectionBox = ({ label, items, selectedId, onSelect, onEdit, onDelete, onAdd, type }) => (
  <div className="bg-white p-2 sm:p-4 rounded-xl border-2 border-slate-300 shadow-sm flex flex-col gap-2 flex-1 min-w-[150px] sm:min-w-[200px]">
    <div className="flex items-center justify-between mb-1">
      <span className="text-[9px] sm:text-[11px] font-black text-slate-600 uppercase tracking-widest line-clamp-2">{label}</span>
    ...
    <select value={selectedId || ''} onChange={(e) => onSelect(e.target.value)} className="w-full p-2 bg-slate-50 border border-slate-300 rounded-lg text-xs sm:text-sm font-bold outline-none cursor-pointer hover:bg-white transition-all">
...
const LoginForm = ({ email, setEmail, password, setPassword, onLogin, toast }) => (
  <div style={{ display:"flex", justifyContent:"center", alignItems:"center", height:"100vh", padding: "20px" }}>
    <div style={{ background:"#ffffff", padding:"25px", borderRadius:"12px", width:"100%", maxWidth: "320px", boxShadow:"0 10px 25px rgba(0,0,0,0.15)", border:"1px solid #eee" }}>
      <h3 style={{ textAlign:"center", fontSize:"18px", fontWeight:"bold", marginBottom:"20px", color:"#4f46e5" }}>Đăng nhập giáo viên</h3>
...
const App = () => {
  ...
  const [showLevel, setShowLevel] = useState(!window.innerWidth < 768);
  const [showNote, setShowNote] = useState(!window.innerWidth < 768);
...
- Specific skills, not generic`;
  ...
  // ===== RENDER =====
  if (!isAuthValid) return <div className="min-h-screen bg-slate-900 flex items-center justify-center p-4 sm:p-6 text-white font-black text-center text-sm sm:text-base">LỖI XÁC THỰC<br/>LIÊN HỆ: 0964567806</div>;
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 pb-20 sm:pb-12 font-sans text-left flex flex-col w-full overflow-x-hidden">
      {toast && <Toast message={toast.message} type={toast.type} icon={toast.icon} />}
      
      {/* HEADER */}
      <header className="bg-indigo-900 text-white p-2 sm:p-1.5 shadow-xl border-b-2 border-indigo-700 w-full sticky top-0 z-40">
        <div className="w-full flex flex-col gap-3 px-3 sm:px-4">
          {/* Logo & Title Row */}
          <div className="flex items-center gap-2 min-w-0">
            <div className="p-1.5 sm:p-2 bg-white/20 rounded-lg flex-shrink-0">
              <img src="./robot.png" className="w-6 sm:w-7 h-6 sm:h-7" alt="Logo" />
            </div>
            <div className="flex flex-col min-w-0 flex-1">
              <div onMouseLeave={()=>setShowUserMenu(false)} className="relative">
                <button 
                  onClick={() => setShowUserMenu(!showUserMenu)} 
                  className="text-xs sm:text-sm font-black text-indigo-200 hover:text-white transition truncate"
                >
                  {user?.displayName || user?.email}
                </button>
                {showUserMenu && (
                  <div className="absolute left-0 top-full mt-1 bg-white rounded-lg shadow-2xl border border-slate-200 overflow-hidden z-50 min-w-max">
                    <button onClick={() => { handleChangeName(); setShowUserMenu(false); }} className="w-full text-left px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 border-b border-slate-100 transition whitespace-nowrap">✏️ Đổi tên</button>
                    <button onClick={() => { handleChangePassword(); setShowUserMenu(false); }} className="w-full text-left px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-100 border-b border-slate-100 transition whitespace-nowrap">🔑 Đổi mật khẩu</button>
                    <button onClick={() => { handleLogout(); setShowUserMenu(false); }} className="w-full text-left px-3 py-2 text-xs font-bold text-red-600 hover:bg-red-50 transition whitespace-nowrap">🚪 Đăng xuất</button>
                  </div>
                )}
              </div>
              <h1 className="text-base sm:text-lg font-black uppercase leading-tight tracking-tight">ĐÁNH GIÁ HỌC SINH</h1>
              <span className="text-indigo-300 text-[7px] sm:text-[9px] font-bold uppercase mt-0.5 italic tracking-wider line-clamp-2">TỐI ƯU HÓA BẰNG AI</span>
            </div>
          </div>

          {/* Control Rows */}
          <div className="flex flex-col gap-2">
            <div className="flex bg-indigo-950/50 p-1 rounded-lg gap-0.5 items-center flex-wrap">
              {['smas', 'vnedu'].map(sys => (
                <button key={sys} onClick={() => { setSystemMode(sys); setSelectedCriteriaId(''); setDraftData({}); }} className={`px-2 sm:px-4 py-1.5 sm:py-2 rounded text-[9px] sm:text-[10px] font-black uppercase transition-all ${systemMode === sys ? 'bg-white text-indigo-900 shadow-lg' : 'text-indigo-300 hover:text-white'}`}>
                  🌐 {sys}
                </button>
              ))}
              <div className="w-px h-4 bg-indigo-700 mx-0.5"></div>
              <div className="flex items-center gap-1 flex-1 min-w-max sm:min-w-0">
                <button onClick={handleSaveAllToFirebase} disabled={isSaving || draftCount === 0} className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-l text-[8px] sm:text-[10px] font-black uppercase transition-all shadow-md active:scale-95 ${draftCount > 0 ? 'bg-amber-500 text-white hover:bg-amber-600 animate-pulse' : 'bg-indigo-800 text-indigo-400 opacity-50 cursor-not-allowed'}`}>
                  {isSaving ? '⏳' : '💾'} <span className="hidden sm:inline">Lưu</span> ({draftCount})
                </button>
                <button onClick={() => setShowApiKeyModal(true)} className={`p-1.5 sm:p-2 rounded-r transition-all text-sm ${apiKey ? 'bg-emerald-600 text-white' : 'bg-red-500 text-white animate-bounce'} shadow-md`}>
                  ⚙️
                </button>
              </div>
            </div>

            <div className="flex bg-indigo-950/50 p-1 rounded-lg gap-1 flex-wrap">
              {['subject', 'competency', 'quality', 'specific'].map(m => (
                <button key={m} onClick={() => { setViewMode(m); setSelectedCriteriaId(''); setDraftData({}); }} className={`px-2 sm:px-3 py-1.5 rounded text-[8px] sm:text-[10px] font-black uppercase transition-all ${viewMode === m ? 'bg-indigo-600 text-white shadow-lg' : 'text-indigo-300 hover:text-white'}`}>
                  {m === 'competency' ? 'NL' : (m === 'subject' ? 'Môn' : (m === 'quality' ? 'PQ' : 'NT'))}
                </button>
              ))}
            </div>
          </div>
      ...
      <div className="w-full px-3 sm:px-4 lg:px-8 py-2 sm:py-3 flex flex-wrap gap-2 sm:gap-4 items-stretch overflow-x-auto pb-2">
        ...
        {viewMode === 'subject' ? (
          <SectionBox label="📚 Môn" items={subjects} selectedId={selectedSubId} onSelect={setSelectedSubId} onEdit={(item) => { setEditItem(item); setInputValue(item.name); setModalType('subject'); }} onDelete={() => setConfirmDelete({ type: 'subject', id: selectedSubId, name: subjects.find(s => s.id === selectedSubId)?.name })} onAdd={() => { setEditItem(null); setInputValue(''); setModalType('subject'); }} type="subject" />
        ) : systemMode === 'vnedu' ? (
          <SectionBox label={viewMode === 'quality' ? '⭐ PQ' : (viewMode === 'competency' ? '🧠 NL' : '👤 ĐT')} items={viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES)} selectedId={selectedCriteriaId} onSelect={setSelectedCriteriaId} onEdit={() => {}} onDelete={() => {}} onAdd={() => {}} type="system_comp" />
        ) : (
          <div className="bg-indigo-50 p-2 rounded-lg border-2 border-indigo-200 shadow-sm flex items-center justify-center font-black text-indigo-700 text-[8px] sm:text-[9px] uppercase gap-1 flex-1 min-w-[100px]">
            {viewMode === 'specific' ? 'ĐẶC THÙ' : (viewMode === 'competency' ? 'CHUNG' : 'PHẨM CHẤT')}
          </div>
        )}
      </div>

      {/* MAIN */}
      <main className="w-full px-3 sm:px-4 lg:px-8 flex-1 overflow-x-hidden">
        {selectedYearId && selectedClassId && selectedMonthId && (viewMode === 'subject' ? selectedSubId : (systemMode === 'vnedu' ? selectedCriteriaId : true)) ? (
          <div className="space-y-3 sm:space-y-4 w-full">
            {/* CONTROLS */}
            <div className="bg-white rounded-xl sm:rounded-2xl p-2 sm:p-4 border-2 border-slate-300 shadow-sm flex flex-col gap-2 sm:gap-3 w-full">
              <div className="flex flex-col gap-2 sm:gap-3">
                <div className="flex-1 w-full text-left">
                  <label className="text-[8px] sm:text-[10px] font-black text-slate-500 uppercase mb-1 block tracking-widest">Hướng dẫn AI</label>
                  <div className="relative">
                    <span className="absolute left-2 sm:left-4 top-1/2 -translate-y-1/2 text-indigo-400 text-base sm:text-lg">✨</span>
                    <input className="w-full bg-slate-50 border-2 border-slate-300 rounded-lg sm:rounded-xl pl-8 sm:pl-12 pr-3 sm:pr-4 py-2 sm:py-3 text-xs sm:text-sm font-semibold outline-none focus:border-indigo-500 transition-all" 
                ...
                <div className="flex gap-2 w-full">
                  <button onClick={runAI} disabled={isGenerating || students.length === 0} className={`flex-1 px-3 sm:px-6 py-2 sm:py-3 ${isGenerating ? 'bg-gray-500' : 'bg-indigo-600 hover:bg-indigo-700'} text-white rounded-lg sm:rounded-xl font-black text-xs flex items-center justify-center gap-1 sm:gap-2 uppercase shadow-lg transition-all active:scale-95 disabled:opacity-50`}>
                    {isGenerating ? '⏳' : '✨'} <span className="hidden sm:inline">{isGenerating ? 'Đang...' : 'AI'}</span>
                  </button>
                  <button onClick={() => { setEditItem(null); setBulkInput(''); setModalType('student'); }} className="p-2 bg-slate-800 text-white rounded-lg shadow-lg hover:bg-black transition-all active:scale-95">👥</button>
                </div>
              </div>

              <div className="flex flex-wrap items-center justify-between pt-2 sm:pt-3 border-t border-slate-100 gap-2">
                <div className="flex gap-2">
                  <button onClick={() => setShowLevel(!showLevel)} className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded text-[8px] sm:text-[10px] font-black uppercase transition-all border ${showLevel ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'}`}>
                    {showLevel ? '👁️' : '🚫'} <span className="hidden sm:inline">Mức</span>
                  </button>
                  <button onClick={() => setShowNote(!showNote)} className={`flex items-center gap-1 px-2 sm:px-3 py-1.5 rounded text-[8px] sm:text-[10px] font-black uppercase transition-all border ${showNote ? 'bg-slate-800 text-white border-slate-800' : 'bg-white text-slate-400 border-slate-200'}`}>
                    {showNote ? '👁️' : '🚫'} <span className="hidden sm:inline">Ghi</span>
                  </button>
                </div>
                <button onClick={exportExcel} disabled={students.length === 0} className="flex items-center gap-1 px-3 sm:px-4 py-1.5 sm:py-2 bg-emerald-600 text-white rounded-lg text-[8px] sm:text-[10px] font-black uppercase hover:bg-emerald-700 shadow-md">
            ...
            {/* TABLE */}
            <div className="bg-white rounded-xl sm:rounded-2xl border-2 border-slate-400 shadow-2xl overflow-hidden mb-12 w-full">
              <div className="overflow-x-auto w-full">
                <table className={`w-full border-collapse ${isMobile ? 'text-xs' : 'text-sm'}`}>
                  <thead>
                    <tr className="bg-slate-200 border-b-2 border-slate-400 sticky top-0">
                      <th className="p-1 sm:p-2 text-center font-black uppercase border-r border-slate-400 whitespace-nowrap text-[8px] sm:text-[10px]">STT</th>
                      <th className="p-1 sm:p-2 text-left font-black uppercase border-r border-slate-400 whitespace-nowrap text-[8px] sm:text-[10px] min-w-[120px]">HS</th>
                      {(systemMode === 'smas' && viewMode !== 'subject') ? (
                        showLevel && (viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES)).map(c => <th key={c.id} className="p-1 text-center font-black uppercase border-r border-slate-400 text-[7px] sm:text-[9px] whitespace-nowrap w-12">{c.name.substring(0, 4)}</th>)
                      ) : (
                        showLevel && <th className="p-1 sm:p-2 text-center font-black uppercase border-r border-slate-400 text-[8px] sm:text-[10px] w-16">Mức</th>
                      )}
                      {showNote && <th className="p-1 sm:p-2 text-center font-black uppercase border-r border-slate-400 text-[8px] sm:text-[10px] min-w-[100px]">Ghi</th>}
                      <th className="p-1 sm:p-2 text-left font-black uppercase text-[8px] sm:text-[10px] min-w-[150px] sm:min-w-[300px]">Nhận xét</th>
                      <th className="p-1 text-center font-black uppercase border-l border-slate-400 text-[8px] sm:text-[10px] w-8 sm:w-10">Z</th>
                  ...
                  <tbody className="divide-y-2 divide-slate-300">
                    {students.map((stu, idx) => {
                      ...
                      return (
                        <tr 
                          key={stu.id} 
                          className={`transition-all ${isInactive ? 'bg-slate-50' : 'hover:bg-indigo-50/50'}`}>
                          
                          <td className={`p-1 sm:p-2 text-center border-r border-slate-400 text-[8px] font-bold sticky left-0 z-10 bg-inherit`}>
                            {idx + 1}
                          </td>
                          
                          <td className="p-1 sm:p-2 border-r border-slate-400 text-left">
                            <button 
                              onClick={() => setStatusModalStudent(stu)} 
                              className={`w-full text-left transition-all text-[8px] sm:text-xs font-black uppercase line-clamp-2 ${isInactive ? 'text-slate-400 line-through' : 'text-slate-700 hover:text-indigo-600'}`}
                              title={stu.name}
                            >
                              {stu.name}
                            </button>
                          ...
                          {(systemMode === 'smas' && viewMode !== 'subject') ? (
                            showLevel && (viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES)).map(c => {
                              ...
                              return (
                                <td key={c.id} className="p-0.5 border-r border-slate-400">
                                  <div className="flex justify-center gap-0.5">
                                    {['T', 'Đ', 'C'].map(lv => (
                                      <button disabled={isInactive} key={lv} onClick={() => updateDraft(stu.id, `level_${c.id}`, lv)} className={`w-4 h-5 rounded text-[7px] font-bold ${lvVal === lv ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'} disabled:opacity-20 transition-colors`}>{lv}</button>
                                    ))}
                                  </div>
                                </td>
                              );
                              ...
                              <td className="p-1 border-r border-slate-400">
                                <div className="flex justify-center gap-0.5">
                                  {(viewMode === 'subject' ? ['T', 'H', 'C'] : ['T', 'Đ', 'C']).map(lv => {
                                    ...
                                    return (
                                      <button disabled={isInactive} key={lv} onClick={() => updateDraft(stu.id, 'level', lv)} className={`w-5 h-6 rounded font-bold text-[8px] ${lvVal === lv ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'} disabled:opacity-20 transition-colors`}>{lv}</button>
                                    );
                                  })}
                                </div>
                              </td>
                          ...
                          {showNote && <td className="p-1 border-r border-slate-400 align-top hidden sm:table-cell"><EditableCell disabled={isInactive} value={finalNote} isDraft={draft.note !== undefined} onSave={(val) => updateDraft(stu.id, 'note', val)} className="bg-transparent text-xs font-bold text-indigo-700 outline-none" /></td>}
                          
                          <td className="p-1 text-left align-top min-w-[100px]">
                            <EditableCell disabled={isInactive} value={finalComment} isDraft={draft.comment !== undefined} onSave={(val) => updateDraft(stu.id, 'comment', val)} className="bg-transparent text-xs font-medium text-slate-700 leading-tight" />
                          </td>
                          
                          <td className="p-1 text-center border-l border-slate-400">
                            <button disabled={!finalComment || isInactive} onClick={() => handleSendZalo(stu.id)} className={`p-1.5 rounded-full mx-auto transition-all text-sm ${copySuccess === stu.id ? 'bg-emerald-100 text-emerald-600' : 'bg-blue-50 text-blue-600 hover:bg-blue-600 hover:text-white disabled:opacity-20'}`}>
                              {copySuccess === stu.id ? '✅' : <ZaloIcon size={14} />}
                            </button>
                          </td>
                          
                          <td className="p-1 text-center"><button onClick={() => setConfirmDelete({ type: 'student', id: stu.id, name: stu.name })} className="text-slate-300 hover:text-red-500 text-base">🗑️</button></td>
                        </tr>
                      );
                    })}
                  </tbody>
        ...
        ) : <div className="py-16 sm:py-32 text-center bg-white rounded-2xl border-4 border-dashed border-slate-300 font-black text-slate-300 uppercase tracking-wider text-xs sm:text-lg mt-4">Chọn đủ dữ liệu để bắt đầu</div>}
      </main>

      {/* FOOTER */}
      <footer className="fixed bottom-0 left-0 w-full px-2 sm:px-4 py-1 bg-white border-t border-slate-200 z-40 text-[7px] sm:text-[10px] text-slate-500">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-1">
          <div className="line-clamp-1">© {new Date().getFullYear()} • {AUTHOR_NAME}</div>
          <div className="text-right italic normal-case line-clamp-1">* Kiểm tra AI trước khi lưu</div>
        </div>
      </footer>

      {/* MODALS - Same as before but with responsive adjustments */}
      {statusModalStudent && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-3 sm:p-4 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white w-full max-w-xs sm:max-w-sm rounded-2xl p-6 sm:p-8 shadow-2xl border-t-8 border-indigo-600">
            {!showMoveTargetSelect ? (
              <>
                <h3 className="text-base sm:text-lg font-black uppercase text-slate-800 mb-2">Trạng thái</h3>
                <p className="text-xs sm:text-sm font-bold text-indigo-600 uppercase mb-6 line-clamp-2">{statusModalStudent.name}</p>
                  ...
                  {Object.keys(STUDENT_STATUS).map(key => {
                    ...
                    return (
                      <button 
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border-2 transition-all text-xs sm:text-sm ${statusModalStudent.status === status.id ? 'bg-indigo-50 border-indigo-600' : 'bg-slate-50 border-transparent hover:border-slate-200'}`}
                      >
                        <span className="text-lg">{status.id === 'active' ? '✅' : status.id === 'dropped' ? '❌' : status.id === 'transferred' ? '✈️' : '➡️'}</span>
                        <span className={`font-black uppercase ${statusModalStudent.status === status.id ? 'text-indigo-900' : 'text-slate-500'}`}>
                          {status.label}
                        </span>
                      </button>
                    );
                ...
                <button onClick={() => setStatusModalStudent(null)} className="w-full py-2 sm:py-3 font-black text-slate-400 uppercase text-[9px] sm:text-[10px]">Đóng</button>
              </>
            ) : (
              <div>
                <div className="flex items-center gap-2 mb-4">
                  <button onClick={() => setShowMoveTargetSelect(false)} className="text-slate-400 hover:text-indigo-600 text-xl">✕</button>
                  <h3 className="text-sm font-black uppercase text-slate-800">Chọn lớp</h3>
                </div>
                <div className="max-h-60 overflow-y-auto space-y-2 mb-6">
                  {classes.filter(c => c.id !== selectedClassId).map(cls => (
                    <button key={cls.id} disabled={isMoving} onClick={() => handleMoveStudentToClass(cls.id)} className="w-full flex items-center justify-between p-3 bg-slate-50 hover:bg-indigo-600 hover:text-white rounded-lg transition-all text-xs sm:text-sm font-bold">
                      <span>{cls.name}</span>
                      <span>→</span>
                    </button>
                  ))}
                </div>
                {isMoving && <div className="flex items-center justify-center gap-2 py-3 text-indigo-600 font-black text-[9px] uppercase">⏳ Đang...</div>}
                <button disabled={isMoving} onClick={() => setShowMoveTargetSelect(false)} className="w-full py-2 font-black text-slate-400 uppercase text-[9px]">Quay lại</button>
              </div>
            )}
          </div>
        </div>
      ...
      {showApiKeyModal && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-3 sm:p-4 bg-slate-900/90 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 sm:p-8 shadow-2xl border-t-8 border-emerald-600">
            <div className="flex items-center gap-2 mb-4">
              <div className="p-2 bg-emerald-100 text-emerald-600 rounded-lg text-xl">🔑</div>
              <h3 className="text-sm sm:text-base font-black uppercase text-slate-800">API Key</h3>
            </div>
            <p className="text-xs sm:text-sm text-slate-600 mb-4">Lấy tại: <a href="https://aistudio.google.com/" target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Google AI Studio</a></p>
            <input type="password" className="w-full p-3 bg-slate-50 border-2 border-slate-300 rounded-lg mb-4 font-mono text-xs outline-none" placeholder="Nhập API Key..." value={apiKey} onChange={e => setApiKey(e.target.value)} />
            <div className="flex gap-3">
              <button onClick={() => setShowApiKeyModal(false)} className="flex-1 py-2 font-black text-slate-400 uppercase text-[9px]">Đóng</button>
              <button onClick={() => saveApiKey(apiKey)} className="flex-[2] py-2 bg-emerald-600 text-white font-black rounded-lg uppercase text-[9px]">Lưu</button>
            </div>
          </div>
        </div>
      )}
      ...
      {modalType && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/80 backdrop-blur-md">
          <div className="bg-white w-full max-w-sm rounded-2xl p-6 sm:p-8 shadow-2xl border-t-8 border-indigo-600">
            <h3 className="text-sm sm:text-base font-black uppercase text-slate-800 mb-4">{editItem ? "Sửa" : "Thêm"}</h3>
            {modalType === 'student' ? 
              <textarea autoFocus className="w-full h-40 p-3 bg-slate-50 border-2 border-slate-300 rounded-lg mb-4 font-bold outline-none text-xs" placeholder="Tên HS (1 tên/dòng)..." value={bulkInput} onChange={e => setBulkInput(e.target.value)}/> 
              : 
              <input autoFocus className="w-full p-3 bg-slate-50 border-2 border-slate-300 rounded-lg font-bold mb-4 outline-none text-xs" value={inputValue} onChange={e => setInputValue(e.target.value)}/>
            }
            <div className="flex gap-2">
              <button onClick={() => setModalType(null)} disabled={isSaving} className="flex-1 py-2 font-black text-slate-400 uppercase text-[9px] disabled:opacity-50">Đóng</button>
              <button onClick={async () => {
                try {
                  ...
                  if (modalType === 'student') {
                    ...
                    showToast(`Thêm ${names.length} HS`, 'success', '✅', 3000);
                  } else {
              ...
              }} disabled={isSaving} className="flex-[2] py-2 bg-indigo-600 text-white font-black rounded-lg uppercase text-[9px] disabled:opacity-50">
                {isSaving ? '⏳' : 'Lưu'}
              </button>
            </div>
          </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-3 sm:p-4 bg-slate-900/90 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xs sm:max-w-sm rounded-2xl p-6 sm:p-8 text-center shadow-2xl border-t-8 border-red-600">
            <h3 className="text-sm sm:text-base font-black mb-3 uppercase">Xác nhận xóa</h3>
            <p className="text-slate-500 text-xs sm:text-sm mb-6">Xóa <span className="text-red-600 font-bold line-clamp-1">"{confirmDelete.name}"</span>?</p>
            <div className="flex gap-3">
              <button onClick={() => setConfirmDelete(null)} className="flex-1 py-2 font-black text-slate-400 uppercase text-[9px]">Hủy</button>
              <button onClick={async () => {
                try {
              ...
              }} className="flex-1 py-2 bg-red-600 text-white font-black rounded-lg uppercase text-[9px]">Xóa</button>
            </div>
          </div>
        </div>
      )}
