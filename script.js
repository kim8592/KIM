...
const App = () => {
  ...
  try {
    ...
    // ===== TEMPLATE NHẬN XÉT CƠ BẢN - CHỈ CHO MÔN HỌC =====
    const commentTemplates = {
      'T': [
        'Em {keyword} rất tốt, thể hiện sự tập trung và hiểu biết sâu sắc.',
      ...
      'H': [
        ...
        'Em thực hiện {keyword} được nhưng còn một số chỗ chưa chắc. Luyện thêm sẽ tốt hơn.',
    ...
    // ===== FUNCTION LẤY THÔNG TIN HỌC SINH =====
    const getStudentInfo = (stu) => {
      ...
      if (viewMode === 'subject') {
        ...
        // ===== SMAS: GỘP TẤT CẢ MỨC + MAP THÀNH VIỆC LÀM =====
        const list = viewMode === 'quality' ? QUALITY_CRITERIA : (viewMode === 'competency' ? GENERAL_COMPETENCIES : SPECIFIC_COMPETENCIES);
    ...
    // ===== TẠO DANH SÁCH NHẬN XÉT CƠ BẢN =====
    const basicComments = {};
    allTargets.forEach(stu => {
      const info = getStudentInfo(stu);

      if (info.level) {
        // ===== CHỈ DÙNG TEMPLATE CHO MÔN HỌC =====
        if (info.type === 'subject' || info.type === 'vnedu_single') {
          ...
          // ===== SMAS: GỬI CHO AI XỬ LÝ - THÊM GHI CHÚ RIÊNG =====
          const selectedKeyword = keywords[Math.floor(Math.random() * keywords.length)];
          basicComments[stu.id] = { 
            comment: null,
            level: info.level,
    ...
    if (!apiKey) {
      ...
      for (const [sId, data] of Object.entries(basicComments)) {
        if (data.comment) {
          ...
          showToast('⚠️ Cần API Key để tạo nhận xét cho năng lực', 'warning', '⚠️', 3000);
        }
      }
    ...
    // ===== GỘP DỮ LIỆU ĐỂ GỬI LÊN AI =====
    let commentsList = "";
    let hasSmAsData = false;

    Object.entries(basicComments).forEach(([sId, data], idx) => {
      if (data.comment) {
        // ===== MÔN HỌC: THÊM GHI CHÚ RIÊNG =====
        const noteStr = data.note ? `|note:[${data.note}]` : '';
        commentsList += `${sId}|${data.comment}${noteStr}\n`;
      } else if (data.data) {
        // ===== SMAS: GỬI VIỆC LÀM/KỸ NĂNG + KEYWORD + GHI CHÚ =====
        const skillsStr = data.data.map(i => `${i.skill}:${i.level}`).join(', ');
        const noteStr = data.note ? `|note:[${data.note}]` : '';
        commentsList += `${sId}|${data.level}|${skillsStr}|keyword:${data.keyword}${noteStr}\n`;
        hasSmAsData = true;
      }
    });

    // ===== PROMPT KHÁC NHAU TÙY THEO LOẠI DỮ LIỆU =====
    let systemPrompt = "";
    let userInstruction = "";

    if (hasSmAsData) {
      // ===== SMAS: AI TỰ TẠO NHẬN XÉT TỪ VIỆC LÀM/KỸ NĂNG =====
      const evalTypeMap = {
        'quality': 'Phẩm chất',
        'competency': 'Năng lực chung',
        'specific': 'Năng lực đặc thù'
      };

      let hasSmAsType = '';
      for (const [sId, data] of Object.entries(basicComments)) {
        if (data.data) {
          hasSmAsType = data.type;
          break;
        }
      }

      systemPrompt = `Bạn là giáo viên tiểu học Việt Nam. Tạo nhận xét học sinh dựa vào mức đạt.
Loại đánh giá: ${evalTypeMap[hasSmAsType] || 'Năng lực'}

RULE:
- Mỗi nhận xét KHÁC NHAU hoàn toàn, độc lập
- NỀU RÕ TẤT CẢ các việc làm/kỹ năng được đánh giá với mức của nó
- NẾU CÓ GHI CHÚ [note], hãy KẾT HỢP ghi chú vào nhận xét để thêm chi tiết và cá nhân hóa
- KẾT HỢP từ khóa (keyword) vào nhận xét sao cho tự nhiên
- Gộp thành nhận xét tự nhiên, mượt mà, khoảng 50 từ
- KHÔNG thêm tên học sinh
- Bắt đầu "Em"
- T: Khen rõ, H/Đ: Khen + hướng phát huy, C: Khen nhẹ + hạn chế + cách sửa

OUTPUT:[ID]|||[Nhận xét 50 từ]`;

      userInstruction = `Dữ liệu (ID|MainLevel|Việc làm/Kỹ năng:Mức|keyword:Từ khóa|note:Ghi chú nếu có):
${commentsList}

Viết nhận xét gộp CHỈ NÊU VIỆC LÀM/KỸ NĂNG (không nêu tên năng lực) cho mỗi em, KẾT HỢP ghi chú riêng nếu có, khoảng 50 từ, mỗi em khác nhau hoàn toàn.`;
    } else {
      // ===== MÔN HỌC: AI SỬA NHẬN XÉT =====
      systemPrompt = `Sửa lại các nhận xét tiểu học cho hay hơn, tự nhiên hơn.
RULE: 
- Giữ nguyên ý, giữ nguyên mức đánh giá
- NẾU CÓ GHI CHÚ [note], hãy KẾT HỢP ghi chú vào nhận xét để thêm chi tiết
- Không thêm tên học sinh, không thêm từ khóa đó
OUTPUT:[ID]|||[Nhận xét hay hơn]`;

      userInstruction = `Các nhận xét cần sửa (có thể kèm ghi chú riêng):
${commentsList}

Hãy sửa lại để hay hơn, tự nhiên hơn, kết hợp ghi chú nếu có.`;
    }
    ...
    if (result.error) {
      ...
      console.warn('⚠️ AI lỗi:', errorMsg);
      ...
      setIsGenerating(false);
      showToast(`⚠️ AI bận, dùng kết quả có sẵn`, 'warning', '⚠️', 3000);
      return;
    }
    ...
    if (!responseText) {
      ...
      setIsGenerating(false);
      showToast(`⚠️ AI không response`, 'warning', '⚠️', 3000);
      return;
    }

    // ===== TẠO HÀM NHẬN XÉT MẶC ĐỊNH CHO SMAS =====
    const generateDefaultSmasComment = (level, items, keyword, note) => {
      const getTItems = items.filter(i => i.level === 'T').map(i => i.skill).join(', ');
      const getHDItems = items.filter(i => i.level === 'H' || i.level === 'Đ').map(i => i.skill).join(', ');
      const getCItems = items.filter(i => i.level === 'C').map(i => i.skill).join(', ');
      
      let comment = '';
      
      if (level === 'T') {
        const templates = [
          `Em xuất sắc với ${getTItems}. Tiếp tục phát huy ${getHDItems ? 'và nâng cao ' + getHDItems : 'để phát triển toàn diện'}.`,
          `Em thể hiện rất tốt ${getTItems}. Duy trì thế mạnh và cần cải thiện ${getCItems || 'thêm các kỹ năng khác'}.`,
          `Em phát triển xuất sắc, đặc biệt ở ${getTItems}. Hãy tiếp tục cố gắng để hoàn thiện hơn.`
        ];
        comment = templates[Math.floor(Math.random() * templates.length)];
      } else if (level === 'H') {
        const templates = [
          `Em hoàn thành tốt với ${getHDItems}. Cần rèn luyện thêm ${getCItems ? getCItems : 'để nâng cao kỹ năng'}.`,
          `Em có tiến bộ tích cực. Tiếp tục nỗ lực để nâng cao các kỹ năng còn hạn chế.`,
          `Em đạt kết quả khá, nhưng cần cố gắng hơn để phát triển toàn diện.`
        ];
        comment = templates[Math.floor(Math.random() * templates.length)];
      } else {
        const templates = [
          `Em cố gắng học tập nhưng cần luyện tập thêm. Hãy chịu khó để tiến bộ rõ hơn.`,
          `Em bước đầu thực hiện được. Cần ôn tập lại và rèn luyện thêm kỹ năng.`,
          `Em có nỗ lực nhưng kết quả chưa như mong đợi. Tiếp tục cố gắng và tìm phương pháp học tốt hơn.`
        ];
        comment = templates[Math.floor(Math.random() * templates.length)];
      }
    ...
    lines.forEach(line => {
      ...
      if (parts.length >= 2) {
        // ===== LẤY ID - CÓ THỂ CHỨA SỐ THỨ TỰ =====
        let sId = parts[0].trim();
        
        // Nếu là số thứ tự (1, 2, 3...), tìm học sinh theo thứ tự
        if (/^\d+$/.test(sId)) {
          const idx = parseInt(sId) - 1;
          if (idx >= 0 && idx < allTargets.length) {
            sId = allTargets[idx].id;
          }
        }
        // Hoặc lấy ID trực tiếp từ basicComments
        else if (!Object.keys(basicComments).includes(sId)) {
          // Thử tìm bằng cách so sánh với các ID trong basicComments
          const foundId = Object.keys(basicComments).find(id => 
            sId.includes(id) || id.includes(sId)
          );
          if (foundId) {
            sId = foundId;
          }
        ...
        if (comment && comment.length > 10 && Object.keys(basicComments).includes(sId)) {
          ...
          if (comment.length > 10) {
            updates[sId] = comment;
            successCount++;
            console.log(`✅ ${sId}: ${comment.substring(0, 60)}...`);
          }
        }
      }
    });

    // ===== DÙNG CƠ BẢN NẾU KHÔNG SỬA ĐƯỢC =====
    const finalUpdates = {};
    for (const [sId, data] of Object.entries(basicComments)) {
      if (updates[sId]) {
        finalUpdates[sId] = updates[sId];
      } else if (data.comment) {
        finalUpdates[sId] = data.comment;
      } else if (data.data) {
        // ===== NẾU AI KHÔNG TRẢ VỀ SMAS, DÙNG TEMPLATE MẶC ĐỊNH =====
        const defaultComment = generateDefaultSmasComment(data.level, data.data, data.keyword, data.note);
        finalUpdates[sId] = defaultComment;
        successCount++;
      }
    }
    ...
    setIsGenerating(false);
    showToast(`✅ Tạo nhận xét: ${successCount}/${Object.keys(finalUpdates).length}`, 'success', '✅', 3000);
    console.log(`✅ Hoàn thành: ${successCount}/${Object.keys(finalUpdates).length}`);
